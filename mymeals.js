...
function addMule() {
   ...
   // document.getElementById('box1').textContent = mules;
};
...
function displayMeal(parseMules){
    for (let i = 0; i < parseMules.length; i++) {
        const MEALS = parseMules[1];
        var mealName = MEALS.name
        if(mealName === mule){

        
       // var recipe = MEALS.recipe;
       // console.log(mealName, recipe);

       mealEl = document.createElement("li");
       mealEl.textContent = mealName;
       // recipe = document.createElement('p');
      //  recipe.textContent = recipe;
    document.getElementById('list-item').append(mealEl);
